...
// 1. Gesture Handler
try {
  const ghDir = path.join(rootDir, 'node_modules', 'react-native-gesture-handler');
  if (fs.existsSync(ghDir)) {
    ...
    const destRuntime = path.join(ghDir, 'android', 'src', 'main', 'jni', 'runtime');
    if (fs.existsSync(srcRuntime)) {
      fs.cpSync(srcRuntime, destRuntime, { recursive: true, force: true });
    }
    const jniCmakePath = path.join(ghDir, 'android', 'src', 'main', 'jni', 'CMakeLists.txt');
    if (fs.existsSync(jniCmakePath)) {
      ...
      if (jniCmake.includes('"${RNGH_DIR}/shared/runtime/*.cpp"')) {
        jniCmake = jniCmake.replace('"${RNGH_DIR}/shared/runtime/*.cpp"', './runtime/*.cpp');
        jniCmake = jniCmake.replace('"${RNGH_DIR}/shared/runtime"', '"${CMAKE_SOURCE_DIR}/runtime"');
...
// 2. Safe Area Context
try {
  const sacDir = path.join(rootDir, 'node_modules', 'react-native-safe-area-context');
  if (fs.existsSync(sacDir)) {
    const srcCommon = path.join(sacDir, 'common', 'cpp', 'react', 'renderer', 'components', 'safeareacontext');
    const destCommon = path.join(sacDir, 'android', 'src', 'main', 'jni', 'safeareacontext');
    if (fs.existsSync(srcCommon)) {
      fs.cpSync(srcCommon, destCommon, { recursive: true, force: true });
    }

    const srcCodegen = path.join(sacDir, 'android', 'build', 'generated', 'source', 'codegen', 'jni');
    const destCodegen = path.join(sacDir, 'android', 'src', 'main', 'jni', 'safeareacontext_codegen');
    if (fs.existsSync(srcCodegen)) {
      fs.cpSync(srcCodegen, destCodegen, { recursive: true, force: true });
    }

    const cmakePath = path.join(sacDir, 'android', 'src', 'main', 'jni', 'CMakeLists.txt');
    if (fs.existsSync(cmakePath)) {
      let cmake = fs.readFileSync(cmakePath, 'utf8');
      cmake = cmake.replace(
...
try {
  ...
  if (fs.existsSync(rnsDir)) {
    const srcCommon = path.join(rnsDir, 'common', 'cpp', 'react', 'renderer', 'components', 'rnscreens');
    const destCommon = path.join(rnsDir, 'android', 'src', 'main', 'jni', 'rnscreens');
    if (fs.existsSync(srcCommon)) {
      fs.cpSync(srcCommon, destCommon, { recursive: true, force: true });
    }

    const srcCodegen = path.join(rnsDir, 'android', 'build', 'generated', 'source', 'codegen', 'jni', 'react', 'renderer', 'components', 'rnscreens');
    const destCodegen = path.join(rnsDir, 'android', 'src', 'main', 'jni', 'rnscreens_codegen');
    ...
    const cmakePath = path.join(rnsDir, 'android', 'src', 'main', 'jni', 'CMakeLists.txt');
    if (fs.existsSync(cmakePath)) {
      let cmake = fs.readFileSync(cmakePath, 'utf8');
      cmake = cmake.replace(
